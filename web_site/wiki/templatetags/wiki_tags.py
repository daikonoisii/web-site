...
@register.inclusion_tag(
    "wiki/includes/render.html", takes_context=True
)
...
@register.inclusion_tag(
    "wiki/includes/form.html", takes_context=True
)
...
@register.inclusion_tag(
    "wiki/includes/messages.html", takes_context=True
)
def wiki_messages(context):
    messages = context.get("messages", [])
    for message in messages:
        message.css_class = settings.MESSAGE_TAG_CSS_CLASS[
            message.level
        ]
...
def get_content_snippet(content, keyword, max_words=30):
    ...
    if match_position != -1:
        try:
            match_start = (
                content.rindex(" ", 0, match_position) + 1
            )
        except ValueError:
            match_start = 0
        try:
            match_end = content.index(
                " ", match_position + len(keyword)
            )
        except ValueError:
        ...
        after_words = all_after[
            : max_words - len(before_words)
        ]
        before = " ".join(before_words)
        after = " ".join(after_words)
        html = (
            "%s %s %s" % (before, striptags(match), after)
        ).strip()
        kw_p = re.compile(
            r"(\S*%s\S*)" % keyword, re.IGNORECASE
        )
...
def is_locked(model):
    ...
    return (
        ...
        and model.current_revision.locked
    )
...
def add_lazy_loading(content):
    ...
    if not content:
        return content
    return content.replace("<img ", '<img loading="lazy" ')


@register.filter
def wiki_render(content):
    """Render wiki content with lazy loading for images."""
