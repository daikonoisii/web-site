...
class ArticleView(ArticleMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 削除済みでない記事一覧
        kwargs['all_article'] = models.urlpath.URLPath.objects.filter(article__current_revision__deleted=0)
        # 最新修正記事の10件取得
        tmp = models.urlpath.URLPath.objects.filter(article__current_revision__deleted=0)
        kwargs["new_modified"] = tmp.order_by("-article__modified")[:10]
        # 記事の執筆者一覧
        articlerevisions = models.article.ArticleRevision.objects.filter(article_id=self.article.id)
        users = []
        for articlerevision in articlerevisions:
            try:
                users.append(str(articlerevision.user))
            except: # 移行時に引き継げなかった
                pass
        kwargs["editors"] = np.unique(users)
...
class Edit(ArticleMixin, FormView):
            ...
            if Form:
                if form_id == self.request.GET.get('f', None):
                    ...
                    if form.is_valid():
                        ...
                        #tag = form.cleaned_data["unsaved_article_tag"]
    ...
    def form_valid(self, form):
        ...
        revision.tag = form.cleaned_data["tag"]
...
class SearchView(ListView):
    ...
    def get_queryset(self):
        ...
        # filter articles by current page path
        ...
        # check if the user has moderation permissions, if not, restrict to reading active articles.
